 ...
 */
async function start(programs, accounts, computeMaxUnits, transactionAccountLockLimit, deactivateFeatures) {
    const ctx = await (0, internal_1.start)(programs.map((p) => [p.name, p.programId.toBytes()]), accounts.map((a) => [a.address.toBytes(), fromAccountInfo(a.info)]), computeMaxUnits, transactionAccountLockLimit, deactivateFeatures?.map((pk) => pk.toBytes()) ?? []);
 ...
 * @returns A container for stuff you'll need to send transactions and interact with the test environment.
 */
async function startAnchor(path, extraPrograms, accounts, computeMaxUnits, transactionAccountLockLimit, deactivateFeatures) {
    const ctx = await (0, internal_1.startAnchor)(path, extraPrograms.map((p) => [p.name, p.programId.toBytes()]), accounts.map((a) => [a.address.toBytes(), fromAccountInfo(a.info)]), computeMaxUnits, transactionAccountLockLimit, deactivateFeatures?.map((pk) => pk.toBytes()) ?? []);
    return new ProgramTestContext(ctx);
}
